...
//    checkChecker();
  ...
  DateTime startDate = DateTime.utc(2018, 01, 01);
  ...
  Future<void> fetchData() async {
    if (await Health.requestAuthorization()) {
      ...
      _healthDataList.forEach((x) {
        if(x.dataType == "STEPS") {
          print("Data point: ${x.value}");
        }
        if(x.dataType == "ACTIVE_ENERGY_BURNED") {
          print("Data point: ${x.value}");
        }
        if(x.dataType == "BASAL_ENERGY_BURNED") {
          print("Data point: ${x.value}");
        }
      });
  ...
  checkChecker() async {
    ...
    DateTime endDate = DateTime.now();
    var _dataList = List<GFHealthData>();

    _isAuthorized = await FlutterHealth.requestAuthorization();
    if(_isAuthorized){
      print('authorized');

      _dataList.addAll(await FlutterHealth.getGFAllData(startDate, endDate));
      var steps = await FlutterHealth.getGFStepCount(startDate, endDate);

      print('printing steps');
      print(steps);
      print(_dataList);

//      _dataList.addAll(await FlutterHealth.getGFBodyFat(startDate, endDate));
    }
  }
